...
// * Type to represent a closed interval (pair of integers)
type ClosedInterval = (i32, i32);

// * We represent the set of intervals as a vector of tuples for simplicty
fn smallest_interval(intervals : &Vec<ClosedInterval>) -> ClosedInterval {
    ...
    // * Loop over all the closed intervals we are given
    ...
    (lowest_upper_bound, highest_lower_bound)
}

#[test]
fn test_smallest_interval1() {
    let intervals : Vec<ClosedInterval> = vec![(0, 3), (2, 6), (3, 4), (6, 9)];
    let result : ClosedInterval = smallest_interval(&intervals);
    let expected : ClosedInterval = (3, 6);
    assert_eq!(result, expected);
}

#[test]
fn test_smallest_interval2() {
    let intervals : Vec<ClosedInterval> = vec![(0, 3), (2, 6), (3, 4), (6, 9), (-5, 0)];
    let result : ClosedInterval = smallest_interval(&intervals);
    let expected : ClosedInterval = (0, 6);
    assert_eq!(result, expected);
}

#[test]
fn test_smallest_interval3() {
    let intervals : Vec<ClosedInterval> = vec![(0, 3), (2, 6), (3, 4), (6, 9), (-5, 2)];
    let result : ClosedInterval = smallest_interval(&intervals);
    let expected : ClosedInterval = (2, 6);
    assert_eq!(result, expected);
}
